...
//        }
  ...
  this.client.execute(sql, params, {prepare: true, consistency: cassandra.types.consistencies.one}, function(err, data) {
...
Cassandra.prototype.generateUUID = function(callback){
  this.client.execute("SELECT now() FROM system.local;", {}, {prepare: true, consistency: cassandra.types.consistencies.one}, function(err, data) {
...
Cassandra.prototype.ping = function(cb) {
  this.execute('SELECT count(*) FROM system.schema_keyspaces', [],{prepare: true, consistency: cassandra.types.consistencies.one}, cb);
};
